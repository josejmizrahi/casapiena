          ...
          <Card key={pa.id} className={cn(pa.excedido && "border-bad/40")}>
            ...
            <CardContent className="pt-0">
              ...
              <div className="flex justify-between gap-2 text-xs text-muted-foreground mt-1.5 num">
                <span>Comprometido <b className="text-foreground">{fm(pa.comprometido)}</b> · pagado {pa.avance}%</span>
                    ...
                    <Row key={c.id} onClick={() => abrir({ tipo: "concepto", d: conceptoForm(c) })} leading={<Dot estado={c.estado} />}
                      left={<>
                        ...
                        <div className="text-xs text-muted-foreground truncate">
                          {nombreProv(c.proveedorId) || "Sin proveedor"}{c.total > 0 ? ` · pagado ${c.pctPagado}%` : ""}{c.logistica !== "porComprar" ? ` · ${LOG[c.logistica]}` : ""}{c.ajustes.length ? " · ajustado" : ""}
                        </div>
                      </>}
