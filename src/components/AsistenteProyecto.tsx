  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && !creando && onClose()}>
      <DialogContent title="Nuevo proyecto" description={`Paso ${paso + 1} de ${PASOS.length} · ${PASOS[paso]}`} className="md:max-w-2xl">
        <ol className="flex gap-1.5 mb-1">{PASOS.map((p, i) => <li key={p} className={cn("h-1 flex-1 rounded-full", i <= paso ? "bg-primary" : "bg-muted")} />)}</ol>
        ...
        {paso === 1 && (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">Elige el tipo de obra. Las partidas son las cajas donde vas a agrupar conceptos; puedes quitar, agregar o renombrar.</p>
            <div className="grid gap-2 sm:grid-cols-2">
              {PLANTILLAS.map((pl) => (
                <button key={pl.id} type="button" onClick={() => elegir(pl)} className={cn("rounded-xl border p-3 text-left transition-colors", plantilla.id === pl.id ? "border-primary bg-accent/60" : "hover:bg-muted")}>
                  ...
                  <div className="text-xs text-muted-foreground mt-0.5">{pl.descripcion}</div>
                </button>
              ))}
            </div>
            <div className="space-y-1.5">
              {filas.map((f, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <Button size="icon" variant="ghost" className="size-8 text-muted-foreground shrink-0" aria-label="Quitar" onClick={() => setFilas((fs) => fs.filter((_, k) => k !== i))}><X /></Button>
                </div>
              ))}
        ...
        {paso === 2 && (
          <div className="space-y-3">
            <div className="rounded-xl bg-gold-bg text-gold px-3 py-2 text-sm flex gap-2"><Lock className="size-4 shrink-0 mt-0.5" /><span><b>El candado es el tope de cada partida.</b> Reparte el presupuesto en porcentajes; la suma no debe pasar del 100 %. Deja imprevistos entre 5 y 10 %. Si después una partida crece, traspásale candado de otra en vez de subirlo.</span></div>
            ...
            <div className="space-y-1.5">
              {filas.map((f, i) => (
                <div key={i} className="grid grid-cols-[1fr_72px_120px] items-center gap-2 text-sm">
                  ...
                  <div className="relative"><Input inputMode="decimal" value={f.pct} onChange={(e) => setPct(i, num(e.target.value))} className="h-9 pr-6 num text-right" /><span className="absolute right-2 top-2 text-xs text-muted-foreground">%</span></div>
            ...
            <div className="rounded-xl border bg-muted/60 px-3 py-1">
              <KV k="Asignado en candados" v={<>{fm(asignado)} <span className="text-muted-foreground font-normal">· {Math.round(totalPct * 10) / 10}%</span></>} tone={totalPct > 100.05 ? "bad" : undefined} />
        ...
        {paso === 3 && (
          <div className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground">Al crearlo te llevo a la vista <b>Hoy</b>, que te dice qué sigue: capturar conceptos en cada partida, asignar proveedores y registrar el primer pago.</p>
          </div>
        )}
